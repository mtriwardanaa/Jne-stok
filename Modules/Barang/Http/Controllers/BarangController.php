...
class BarangController extends Controller
{
    ...
    public function stok($id, $stok) {
        $check = Barang::where('id', $id)->first();

        if (empty($check)) {
            return back()->withErrors(['Barang tidak di temukan'])->withInput();
        }

        $total = $stok - $check->qty_barang;
        $check->stok_awal = $total;
        $check->update();
        if (!$check) {
            return back()->withErrors(['Update stok barang gagal']);
        }

        return redirect('barang')->with(['success' => ['Ubah stok barang berhasil']]);
    }
